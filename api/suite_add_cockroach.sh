...
do
    ...
    if [ -f "json/${SERVICE_NAME}.json" ]; then
        ./importDBService.sh "json/${SERVICE_NAME}.json"
    else
        echo "⚠️  Warning: ${SERVICE_NAME}.json not found. Skipping service import."
    fi

    # Import Pipelines
    if [ -f "json/${SERVICE_NAME}_pipelines.json" ]; then
        ./importPipelines.sh "json/${SERVICE_NAME}_pipelines.json"
    else
