...
if [ -z "$INPUT_FILE" ] || [ ! -f "$INPUT_FILE" ]; then
    ...
    echo "Usage: ./importSearchService.sh <exported_service.json>"
    exit 1
fi
...
echo "🚀 Importing Search Service from: $INPUT_FILE"
...
RESPONSE=$(curl -s -L -X POST "${BASE_URL}/services/searchServices" \
...
if [ ! -z "$NEW_ID" ] && [ "$NEW_ID" != "null" ]; then
    echo "✅ Search Service successfully imported!"
    echo "🆔 New Service ID: $NEW_ID"
    echo "🔗 Service Name: $(echo "$RESPONSE" | jq -r '.name')"
else
    echo "❌ Failed to import Search Service."
